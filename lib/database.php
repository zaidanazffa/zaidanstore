...
//Count Layanan
$count_layanan_sosmed = mysqli_num_rows($conn->query("SELECT * FROM layanan_sosmed"));
$count_layanan_pulsa = mysqli_num_rows($conn->query("SELECT * FROM layanan_pascabayar"));
$count_layanan_pascabayar = mysqli_num_rows($conn->query("SELECT * FROM layanan_pascabayar"));
//Total Pemesanan
...
// Keuntungan //
    $rate = $conn->query("SELECT * FROM setting_rate WHERE tipe = 'Sosial Media'");
    $untungg = $rate->fetch_assoc();
    $profit_sosmed = $untungg['rate']/1000;
