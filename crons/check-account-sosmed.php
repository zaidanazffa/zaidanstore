...
$sisa_saldo_sosmed = $json_result['data']['sisa_saldo_sosmed'];
$indeks++; 
$i++;

        $cek_akun = $conn->query("SELECT * FROM cek_akun WHERE provider = 'SULTAN'");
        $data_akun = mysqli_fetch_assoc($cek_akun);
        if (mysqli_num_rows($cek_akun) > 0) {
        $update = $conn->query("UPDATE cek_akun SET saldo = '$sisa_saldo_sosmed', date = '$date', time = '$time' WHERE provider = 'SULTAN'");
...
$insert = $conn->query("INSERT INTO cek_akun VALUES ('','$sisa_saldo_sosmed','$date','$time','SOSIAL MEDIA','SULTAN')");//Memasukan Kepada Database (OPTIONAL)
if ($insert == TRUE) {
