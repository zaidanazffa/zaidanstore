		...
		if (mysqli_num_rows($cek_usernya) == 1) {
		    ...
		    if ($provider == "MANUAL") {
			    $api_postdata = "";
			                        ...
			                        $api_postdata = "api_key=".$data_provider['api_key']."&action=pemesanan&layanan=$layanan&target=$target&jumlah=$jumlah";
			    ...
			    if ($provider == "SULTAN" AND $json_result['error'] == true) {
			        $hasilnya = array('status' => false, 'data' => array('pesan' => 'Ups, '.$json_result['error'].''));
			    } else {
			        ...
			        if ($provider == "SULTAN") {
		                $provider_oid = $json_result['data']['id'];
									...
									if ($conn->query("INSERT INTO pembelian_pulsa VALUES ('','API-$order_id', '$provider_oid', '".$datanya['username']."', '".$data_layanan['layanan']."', '".$data_layanan['harga_api']."', '".$data_rate['rate']."', '$koin', '$target', '', 'Pending', '$date', '$time', 'API', '$provider', '0')") == true) {
										...
										$hasilnya = array('status' => false, 'data' => array('pesan' => 'Ups, Gagal! Sistem Kami Sedang Mengalami Gangguan1.'));
									}
								}
							}
						}
					...
					$hasilnya = array('status' => false, 'data' => array('pesan' => 'Ups, Gagal! Sistem Kami Sedang Mengalami Gangguan2.'));
				}
