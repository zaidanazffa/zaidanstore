						...
						if (mysqli_num_rows($cek_layanan) == 0) {
							...
							if ($jumlah < $data_layanan['min']) {
								...
								if ($provider == "") {
			                        ...
			                        $post_datanya = "api_key=".$data_provider['api_key']."&action=pemesanan&layanan=$layanan&target=$target&jumlah=$jumlah";
								...
								if ($provider == "SULTAN" AND $resultnya['status'] == false) { 
                    			    $hasilnya = array('status' => false, 'data' => array('pesan' => 'Ups, '.$resultnya['data']['pesan']));
								} else {
